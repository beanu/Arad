...
import com.beanu.arad.pulltorefresh.BuildConfig;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	protected void setRefreshingInternalNow(boolean doScroll) {
		mState = REFRESHING;
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				smoothScrollTo(mCurrentMode == Mode.PULL_DOWN_TO_REFRESH ? -mHeaderHeight : mHeaderHeight, 10);
				// smoothScrollTo(-98, 10);
	...
	protected void updateUIForMode() {
		...
		if (mMode.canPullDown()) {
			addViewInternal(mHeaderLayout, 0, new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
		...
		if (mMode.canPullUp()) {
			addViewInternal(mFooterLayout, new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
					ViewGroup.LayoutParams.WRAP_CONTENT));
		}
	...
	private void addRefreshableView(Context context, T refreshableView) {
		...
		addViewInternal(mRefreshableViewWrapper, new LayoutParams(LayoutParams.MATCH_PARENT, 0, 1.0f));
	}
	 ...
	 * {@link org.qii.weiciyuan.support.lib.pulltorefresh.PullToRefreshAdapterViewBase#setOnLastItemVisibleListener}
	 * .
	 * 
	 * @author Chris Banes
	 */
