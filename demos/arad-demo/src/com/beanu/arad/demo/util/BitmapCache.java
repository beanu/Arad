...
package com.beanu.arad.demo.util;
...
public class BitmapCache implements ImageLoader.ImageCache {
	 ...
	 *            memory cache that needs to be retained).
	 * @param memCacheSize
	 *            Memory cache size in KB.
	 */
	...
	private void init(int memCacheSize) {
		// Set up memory cache
		Log.d("Memory cache created (size = " + memCacheSize + "KB)");
	...
	public Bitmap getBitmapFromMemCache(String data) {
		if (data != null) {
			synchronized (mMemoryCache) {
				...
				if (memBitmap != null) {
					Log.d("Memory cache hit - " + data);
					return memBitmap;
				}
			}
			Log.d("Memory cache miss - " + data);
		}
		return null;
	}
