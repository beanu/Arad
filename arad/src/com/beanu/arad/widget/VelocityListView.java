 ...
 * 
 * <p>
 * Date: 13-3-31 modify to add int currentItemCount = totalItemCount; int v =
 * currentItemCount - mLastItemCount; mFirstVisiblePosition += v;
 * mLastVisiblePosition += v; so, when notifyDataSetChanged adapter to show new
 * item, the Velocity is also correct.
 * 
 * <p>
 * and modify setOnScrollListener method, so that ListView can own more than
 * just one OnScrollListener
 * 
...
public class VelocityListView extends AutoScrollListView {
	...
	private OnScrollListener mOnScrollListener = new OnScrollListener() {
		...
		public void onScroll(AbsListView view, int firstVisiblePosition, int visibleItemCount, int totalItemCount) {
			...
			// calc position,because ListView may has new item because adapter
			// has something new
